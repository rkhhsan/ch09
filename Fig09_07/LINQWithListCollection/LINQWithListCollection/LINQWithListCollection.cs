...
public class LINQWithListCollection
{
   public static void Main(string[] args)
   {
      ...
      */
      ...
      // convert all strings to lowercase; select those starting with "R"
      var lowercaseWords =
         from item in items
         let lowercaseString = item.ToLower()
         orderby lowercaseString descending
         select lowercaseString;

      // display query results
      foreach (var item in lowercaseWords)
         Console.Write("{0} ", item);
